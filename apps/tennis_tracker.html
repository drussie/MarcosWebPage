...
<body class="min-h-screen bg-slate-50 text-slate-900">
  <div class="max-w-6xl mx-auto p-4 md:p-6 lg:p-8">
    <header class="mb-6 flex items-start md:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl md:text-3xl font-bold">🎾 Tennis Score & Stats Tracker</h1>
      ...
      <form id="pointForm" class="grid md:grid-cols-2 gap-6">
        <div class="space-y-4">
          ...
          <div>
            ...
            <select name="serveResult" class="mt-1 w-full px-3 py-2 rounded-xl border">
              ...
              <option value="1stout_2ndin">1st out → 2nd in</option>
        ...
        <div>
          ...
          <ul class="mt-2 text-sm list-disc pl-5 space-y-1 text-slate-700">
            <li>Use <em>Serve → Double fault</em> for DF; returner automatically wins the point.</li>
  ...
  function finalizePoint(point) {
    ...
    document.getElementById('lastPoint').textContent = `#${n}: ${pName} won the point (${point.outcome} – ${cat}; serve: ${sr})${point.comment?` — ${point.comment}`:''}`;
  ...
  function renderScoreboard() {
    ...
    const row = (idx) => {
      ...
      const isServer = state.server===idx ? ' 🔸' : '';
    ...
    if (state.settings && state.settings.format==='pro') {
      ...
      hints.textContent = `Sets: ${sw[0]}–${sw[1]} (first to ${needToWin}). Server marked with 🔸.`;
    }
  }
  ...
  function renderPointLog() {
    ...
    const rows = state.history.map((p, i) => {
      ...
      return `<div class="p-3 border rounded-xl">
        <div class="text-sm"><span class="text-slate-500">#${i+1}</span> <span class="font-medium">${winner}</span> won — ${p.outcome} by ${by} • ${cat} • serve ${serveTxt}${forced}</div>
        ${c}
      </div>`;
    });
  ...
  // ---------- Dynamic defaults for point form ----------
  function updateDynamicDefaults() {
    ...
    if (endType === 'winner') {
      if (bySel) bySel.value = String(sIdx);
      if (catSel) catSel.value = 'serve';
    } else if (endType === 'error') {
      if (bySel) bySel.value = String(rIdx);
    }
  }
